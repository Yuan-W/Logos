...
from src.agents.tools import dice_roller, create_rule_lookup, update_character, lookup_stats
...
def route_by_intent(state: GameState) -> Literal["handler", "narrator"]:
...
def build_gm_agent(llm: BaseChatModel, session: Session, checkpointer: Optional[Any] = None, glossary_context: str = "") -> CompiledStateGraph:
    ...
    # Intent router: ACTION goes to handler, others go directly to storyteller
    graph.add_conditional_edges("intent_parser", route_by_intent, {
        "handler": "handler",
        "narrator": "storyteller"  # DIALOGUE/QUERY -> storyteller directly
    })
